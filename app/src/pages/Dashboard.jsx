...
const Dashboard = () => {
  ...
  // ✅ Organized & grouped menu
  const menuItems = [
    // 📚 BOOK MANAGEMENT
    { name: "📚 BOOK MANAGEMENT", isHeader: true },
    ...
    { name: "🌍 General Users", path: "/general-users" },
  ...
  const getFullMenu = () => {
    ...
    if (userRole === "superadmin") {
      finalMenu.unshift(
        { name: "🛡️ SUPERADMIN", isHeader: true },
        { name: "🛡️ Admin Management", path: "/admin-management" },
        { name: "💰 Financial Vault", path: "/financial-vault" },
      ...
      <div className="w-64 bg-green-700 text-white flex flex-col p-4 shadow-lg justify-between overflow-y-auto">
            ...
            {getFullMenu().map((item, index) => {
              if (item.isHeader) {
                return (
                  <div
                    key={`header-${index}`}
                    className="text-xs text-yellow-300 mt-4 mb-1 px-2 font-bold uppercase tracking-wider"
                  >
                    {item.name}
                  </div>
                );
        ...
        <div>
          <div className="text-[10px] text-green-300 text-center mb-2 uppercase tracking-tighter">
            Logged in as:{" "}
            <span className="text-white font-bold">{userRole || "User"}</span>
          </div>

          <button
            onClick={handleLogout}
            className="w-full bg-red-500 text-white py-2 rounded-xl font-bold hover:bg-red-600 transition text-xs shadow-inner"
          >
            🚪 Logout to Home
          </button>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center text-center p-10">
          <div className="bg-white p-12 rounded-3xl shadow-xl border-2 border-yellow-100 max-w-2xl relative overflow-hidden">
            ...
            <p className="text-lg text-gray-600 mb-8">
              System active. Select a task from the sidebar.
            </p>

            <p className="text-xl text-red-600 font-black animate-pulse uppercase tracking-widest">
              🧭 Dashboard Ready
            </p>
          </div>
        </div>
